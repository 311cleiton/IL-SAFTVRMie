...
full_file_path_mCp = ["C:/Users/cleiton/My Drive/usp-brainstorm/bs04/csv/Cp/mix_mCp.csv"]
full_file_path_T = ["C:/Users/cleiton/My Drive/usp-brainstorm/bs04/csv/Cp/mix_T.csv"]
...
#############
P_1 = 101.325E+03 # Pa
exp_data = 11 # experimental datasets evaluated
temperature_model_min = 260 # K
temperature_model_max = 440 # K
delta_temperature_model = temperature_model_max - temperature_model_min # K
temperature_model_x_axis = [temperature_model_min,temperature_model_max] # K
# Non-associating
...
all_exp_T = []
for i ∈ 1:exp_data
    valid_mCp = dropmissing(expdata_mCp,[i]) # remove missing values from DataFrame
...
model_mCp = [] # g/L == kg/m³
for i ∈ 1:n_models
    append!(T,[range(temperature_model_min,temperature_model_max,delta_temperature_model)])   
    m_Cp = isobaric_heat_capacity.(models[i], P_1, T[i])
    append!(model_mCp,[[m_Cp[i][1] for i ∈ 1:delta_temperature_model]])
end
#
...
PyPlot.xlim(temperature_model_x_axis)
# PyPlot.ylim([260,470])
PyPlot.yticks(collect(250:50:450))
display(PyPlot.gcf())
...
PyPlot.xlim(temperature_model_x_axis)
# PyPlot.ylim([310,790])
PyPlot.yticks(collect(300:100:800))
display(PyPlot.gcf())
#
...
PyPlot.xlim(temperature_model_x_axis)
# PyPlot.ylim([530,890])
PyPlot.yticks(collect(550:50:850))
PyPlot.ylim([550,850])
display(PyPlot.gcf())
#
